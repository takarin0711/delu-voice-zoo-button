...
### 音声ファイルの管理
音声ファイルは `voice/` フォルダにまとめて配置。`manifest.json` の `web_accessible_resources` は `voice/*.mp3` で一括指定しているため、ファイルを追加しても `manifest.json` の変更は不要。ただし `SOUNDS` 配列は `content.js`（拡張機能用）と `page.js`（ウェブページ用）の2箇所に存在するため、音声を追加する際は**両方**を更新すること。

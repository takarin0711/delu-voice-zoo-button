...
- CSS（外部ライブラリなし）
  - ウェブページ版 `page.css` のみ Google Fonts（M PLUS Rounded 1c）を `@import` で使用
...
### waitForElement のリトライとキャンセル
`waitForElement` はタイムアウト後に最大3回リトライする（`retries` パラメータ）。ページ遷移時（`yt-navigate-finish`）に `onNavigate` が `waitCancel()` を呼び出すことで、古いリトライを打ち切る。`waitCancel` はグローバル変数として保持し、`waitForElement` が呼ばれるたびに上書きされる。リトライ・キャンセル処理を変更する際はこの仕組みを維持すること。

### removeButton時のイベントリスナー解除
`injectButton` 内で `document` に登録した `onOutsideClick` リスナーは、`removeButton()` 実行時にも解除する必要がある。コンテナ要素に `container._closePanel = closePanel` として参照を保持し、`removeButton()` で `container._closePanel()` を呼び出すことで解除している。パネル開閉処理を追加・変更する際はこの仕組みを維持すること。
...
### Canvas 星空背景（ウェブページ版）
`index.html` の `<canvas id="starfield">` に星空をリアルタイム描画している。描画ロジックは `page.js` 先頭の即時実行関数にまとめている。星のサイズ・色・瞬き速度はランダム生成で、天の川帯（対角方向の星密度集中）・明るい星の回折スパイク・流れ星（9〜24秒ごと）を含む。`page.css` の `body::before` は大気グロー（CSS）のみを担い、星の描画は行っていない。
